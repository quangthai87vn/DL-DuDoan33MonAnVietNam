# app1.py — VN Foods Streamlit (EfficientNet-B0, chuẩn notebook + Random-from-Test)
import os, io, zipfile, warnings, random
...
def list_images_recursively(root: Path) -> List[Path]:
    """Lấy tất cả file ảnh trong cây thư mục (Test/<class>/*.jpg...)."""
    if not root.exists():
        return []
    paths = []
    for p in root.rglob("*"):
...
def load_images_from_paths(paths: List[Path]) -> List[Tuple[str, Image.Image]]:
    out = []
    for p in paths:
        try:
...
# Nguồn ảnh đầu vào
src = st.sidebar.radio("Nguồn ảnh", ["Upload ảnh/ZIP", "Random n ảnh từ thư mục Test"], index=0)
...
try:
    # tạo model tạm để đọc class_names (nếu ckpt có)
...
if src == "Upload ảnh/ZIP":
    ...
    st.subheader("Lấy ngẫu nhiên n ảnh từ thư mục **Test** (dạng Test/<class>/*.jpg)")
    # Gợi ý đường dẫn mặc định theo bạn đang dùng trong notebook
    default_test_dir = "/media/mtl/DATA 6TB/AI DATASET/vietnamese-foods/Images/Test"
    test_dir_str = st.text_input("Đường dẫn thư mục Test:", value=default_test_dir)
    n_random = st.number_input("Số ảnh ngẫu nhiên", min_value=1, max_value=64, value=12, step=1)
    go = st.button("🎲 Chọn & Dự đoán")

    if go:
        test_dir = Path(test_dir_str)
        all_paths = list_images_recursively(test_dir)
        if not all_paths:
            st.error("Không tìm thấy ảnh trong thư mục Test (hoặc đường dẫn sai).")
        else:
            pick = random.sample(all_paths, min(int(n_random), len(all_paths)))
            images = load_images_from_paths(pick)
            st.info(f"Đã lấy {len(images)} ảnh ngẫu nhiên từ: `{test_dir}`")

if not images:
    st.info("👉 Cung cấp ảnh (hoặc bấm nút chọn ảnh ngẫu nhiên từ Test) để dự đoán.")
    st.stop()
